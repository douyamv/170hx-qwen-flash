...
// NEXT: also used on CUDA when cub::DeviceTopK is unavailable (the argsort fallback sorts the whole row per query)
...
// NEXT: deterministic gather. Each block owns a contiguous range of the row, so with per-block prefix counts the
...
#elif defined(GGML_CUDA_USE_CUB)  // CUB_TOP_K_AVAILABLE
    // NEXT: radix select instead of sorting the whole row (qwen4exp QSA: k=2051 of up to 262144 per query)
    static const bool force_sort = getenv("NEXT_TOPK_SORT") != nullptr;
