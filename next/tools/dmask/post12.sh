...
for N in (70000, 200000):
    ...
    out = '/mnt/slowdisk/tests-archive/traces/cpu-v62-%dk.csv' % (N // 1000)
    os.rename(P + '/logs/profile.csv', out) if False else open(out, 'wb').write(open(P + '/logs/profile.csv', 'rb').read())
    print('saved', out)
PY
: > $P/logs/profile.csv
for n in 70 200; do echo "== host timeline ${n}K =="; python3 $T/host/an_cpu.py /mnt/slowdisk/tests-archive/traces/cpu-v62-${n}k.csv 1 2>&1 | head -n 48 | cut -c1-120; done
echo "[post12] done $(date +%T)"
