...
LJ=${LJ:-launch.json.v4b}   # v4b = v3 + NEXT_TOPK_NOSORT=1 (async input uploads left off: races with set_inputs across ubatches)
[ -f $L/libggml-cuda.so.0.22.0 ] && [ -f $P/$LJ ] || { echo "missing $L or $LJ"; exit 1; }
