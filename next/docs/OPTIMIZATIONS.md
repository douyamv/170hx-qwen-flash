...
| 16 | `moea`: expert-grouped MoE GEMV (`mul_mat_id`, ≤ 8 tokens): tokens that selected an expert are grouped with the existing `mm_ids_helper`, the block streams the expert's rows once with 16-byte (Q4_K) / 8-byte (Q5_1) loads; fused up·silu(gate). Off: `NEXT_MOEA=0` | `ggml/src/ggml-cuda/moea.{cu,cuh}`, `mmvq.cu` | under validation |
